...
# Safely pulls the key from the Advanced Settings Secrets box you configured
...
if user_input := st.chat_input("Type your house requirements here..."):
    ...
    for idx, h in enumerate(all_houses):
        database_context += f"- Plan {h.get('3D_Folder_Path')}: {h.get('NO OF BEDROOM')} Bedrooms, {h.get('NO OF BATHROOM')} Bathrooms, Price: RM {h.get('Price_RM')}, Built-up: {h.get('Built_up_sqft')} sqft. Dimensions: Kitchen {h.get('KITCHEN SIZE')}, Living {h.get('LIVING AREA')}.\n"

    system_prompt = (
        "You are an expert house plan consultant chatbot. Your task is to analyze the user's message, "
        "look at the provided house plans database, and recommend the best plan options. "
        "Be conversational, professional, and friendly. Speak in English or Malay depending on how the user greets you. "
        "CRITICAL RULE: If you recommend any specific plans, you MUST list their exact plan IDs (e.g., 3D_Models/P01, 3D_Models/P04) at the very bottom of your message "
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing requirements..."):
            try:
                ...
                # Official Python SDK Client call prevents cloud authentication rejection
                ...
                if "[MATCHES:" in ai_text:
                    ...
                    for house in all_houses:
                        if house.get('3D_Folder_Path') in mentioned_ids:
